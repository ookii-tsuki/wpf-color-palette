...
namespace OokiiTsuki.Palette
{
    public class Palette
    {
        ...
        public static Palette Generate(BitmapSource image, int numColors = DEFAULT_CALCULATE_NUMBER_COLORS)
        {
            if (numColors < 1)
            ...
            BitmapSource scaledTexture = ScaleTextureDown(image);
        ...
        private static BitmapSource ScaleTextureDown(BitmapSource image)
        {
            // Scale texture to fit max size preserving aspect ratio

            var maxResizeFactor = Math.Min(CALCULATE_TEXTURE_MIN_DIMENSION / (float)image.PixelWidth, CALCULATE_TEXTURE_MIN_DIMENSION / (float)image.PixelHeight);

            if (maxResizeFactor > 1)
                return image;

            var width = (int)(maxResizeFactor * image.PixelWidth);
            var height = (int)(maxResizeFactor * image.PixelHeight);

            var targetBitmap = new TransformedBitmap(image, new ScaleTransform(maxResizeFactor, maxResizeFactor));
     ...
     */
        ...
        public class Swatch
        {
            ...
            public Swatch(int red, int green, int blue, int population)
            {
                ...
                Green = green;
                Blue = blue;
                Rgb = Color.FromArgb(255, (byte)red, (byte)green, (byte)blue).ToInt();
                Population = population;
            }

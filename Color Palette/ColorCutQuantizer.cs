...
namespace OokiiTsuki.Palette
{
    public class ColorCutQuantizer
    {
        ...
        private static PixelColor[,] CopyPixels(BitmapSource source)
        {
        ...
        ///<param name="image">Texture to extract the pixel data from</param>
        ///<param name="maxColors">The maximum number of colors that should be in the result palette.</param>
        public static ColorCutQuantizer FromTexture2D(BitmapSource image, int maxColors)
        {
            ...
            for (int y = 0; y < image.PixelHeight; y++)
            {
                ...
                {
                    byte a = pixels[x, y].Alpha;
                    byte r = pixels[x, y].Red;
                    byte g = pixels[x, y].Green;
                    byte b = pixels[x, y].Blue;
                    intPixels[y * image.PixelHeight + x] = Color.FromArgb(a, r, g, b).ToInt();
                }
            }
        ...
        private static void ModifySignificantOctet(int dimension, int lowIndex, int highIndex)
        {
            switch (dimension)
            {
                ...
                case COMPONENT_GREEN:
                    ...
                    {
                        int color = colors[i];
                        colors[i] = Color.FromArgb(255, (byte)color.Green(), (byte)color.Red(), (byte)color.Blue()).ToInt();
                    }
                    break;
                case COMPONENT_BLUE:
                    ...
                    {
                        int color = colors[i];
                        colors[i] = Color.FromArgb(255, (byte)color.Blue(), (byte)color.Green(), (byte)color.Red()).ToInt();
                    }
                    break;
            }
        }
